...
from data import imu_preprocessing
...
def initialize_syn_pseudo_with_dido(
    model: SyntheticMotionClassifier, traj_dict: dict, idx: int
):
...
def run_filter(args):
    ...
    trajectories = data_loaders.CloudTrajectoryDataset(args=args)
    ...
    for i, traj in enumerate(trajectory_dataloader, 0):

        f = traj[0][0]
        traj_dict = traj[1]

        quadrotor_model = SyntheticMotionClassifier(args, traj_dict["v"][0].shape[0])
        ...
        for m_k, t_k, z_k in tqdm(
            ...
            if not quadrotor_model.initialized:
                ...
                quadrotor_model.update(z_k)
                quadrotor_model.predict(m_k, dt)
                quadrotor_model.correct(m_k, dt)
            ...
            shifted_quad_marker = quadrotor_model.marker[torch.Tensor([0, 2, 3, 4]).int()]
            if torch.any(quadrotor_model.marker):
                log_vec_k = torch.cat(
                    (
                        ...
                        t_k.view(1, 1, 1),
                        shifted_quad_marker.reshape(1, 4, 1),
                        omega,
                        acc,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", type=str, default="cloud_dataset/")
    parser.add_argument(
        "--file_target",
        type=str,
        default='utiascircle_trial1/repeat',
    )
    parser.add_argument("--filter_output_name", type=str, default="syn_pseudo_right_forwards_utias_field.txt")
    ...
    parser.add_argument("--sigma_z_vel_null", type=float, default=1.0)
    parser.add_argument("--sigma_z_vel_rp", type=float, default=0.4)
    parser.add_argument("--sigma_omega", type=float, default=0.04)
    parser.add_argument("--sigma_z_vel_planar", type=float, default=1.) # 3
    parser.add_argument("--sigma_z_vel_lateral", type=float, default=1.) # 6
    parser.add_argument("--innovation_threshold", type=float, default=1.) # 0.1
