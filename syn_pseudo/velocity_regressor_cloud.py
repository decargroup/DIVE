...
def initialize_syn_pseudo_with_dido(
    model: VelocityUnitVectorRegressor, traj_dict: dict, idx: int
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file_target",
        type=str,
        default="utiasfield_trial1/teach",
    )
    parser.add_argument(
        "--filter_output_name", type=str, default="velRegLeft_fullAug.txt"
    )
    parser.add_argument("--ground_truth_output_name", type=str, default="data.hdf5")
    parser.add_argument(
        "--model_path",
        type=str,
        default="/home/angad/learned_quad_inertial/learned_quad_inertial_odometry/lightning_logs/validation_checkpoints/velVecRegressor_fullAug_corrected_best_val_loss.ckpt", 
    ) 
    ...
    parser.add_argument("--sigma_gyro_ct", type=float, default=1e-4)
    parser.add_argument("--sigma_accel_ct", type=float, default=1e-3)
    parser.add_argument("--sigma_gyro_bias_ct", type=float, default=1e-5)
    parser.add_argument("--sigma_accel_bias_ct", type=float, default=1e-4)

    parser.add_argument("--sigma_velocity_init", type=float, default=.25)
    parser.add_argument(
        "--sigma_theta_rp_init", type=float, default=.5 * torch.pi / 180
    )
    parser.add_argument(
