...
from modelling.quad import VelocityUnitVectorRegressor
...
def run_filter(args):
    ...
    for i, traj_tuple in enumerate(trajectory_dataloader, 0):
        ...
        quadrotor_model = VelocityUnitVectorRegressor(args, traj_dict["v"][0].shape[0])
        ...
        X_k_hist = torch.empty((measurements.shape[0], 9, 1))
        ...
        for m_k, t_k, gt_k in tqdm(
            ...
            # print(quadrotor_model.P.diagonal(dim1=2)[:, 1])
            ...
            X_k_hist[quadrotor_model.idx] = quadrotor_model.agg_x[:, :9, :]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--filter_output_name", type=str, default="velocity_unit_regressor_left.txt"
    )
    parser.add_argument("--ground_truth_output_name", type=str, default="data.hdf5")
    parser.add_argument(
        "--model_path",
        type=str,
        default="/home/angad/learned_quad_inertial/learned_quad_inertial_odometry/lightning_logs/validation_checkpoints/vu-regressor-best-val-loss.ckpt",
    ) 
